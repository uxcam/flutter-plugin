...
/// A place in an [RouteOverlay] that can contain a widget.
///
/// Overlay entries are inserted into an [RouteOverlay] using the
/// [RouteOverlayState.insert] or [RouteOverlayState.insertAll] functions. To find the
/// closest enclosing overlay for a given [BuildContext], use the [RouteOverlay.of]
/// function.
///
...
///
/// Because an [RouteOverlay] uses a [Stack] layout, overlay entries can use
...
///    [RouteOverlay] using a builder callback.
///  * [RouteOverlay], a stack of entries that can be managed independently.
///  * [RouteOverlayState], the current state of an Overlay.
...
class OverlayEntry implements Listenable {
  ...
  ///
  /// To insert the entry into an [RouteOverlay], first find the overlay using
  /// [RouteOverlay.of] and then call [RouteOverlayState.insert]. To remove the entry,
  ...
  void remove() {
    assert(_overlay != null);
    assert(!_disposedByOwner);
    final RouteOverlayState overlay = _overlay!;
    _overlay = null;
    if (!overlay.mounted) {
      return;
    }
...
class _OverlayEntryWidget extends StatefulWidget {
  ...
  final RouteOverlayState overlayState;
...
///
/// Although you can create an [RouteOverlay] directly, it's most common to use the
...
///
/// The [RouteOverlay] widget uses a custom stack implementation, which is very
/// similar to the [Stack] widget. The main use case of [RouteOverlay] is related to
...
///
/// An [RouteOverlay] widget requires a [Directionality] widget to be in scope, so
...
/// [RouteOverlay] is the size returned by [MediaQuery.sizeOf]. Nested overlays can
/// have different sizes.
///
/// {@tool dartpad}
/// This example shows how to use the [RouteOverlay] to highlight the [NavigationBar]
/// destination.
///
...
///  * [RouteOverlayState], which is used to insert the entries into the overlay.
///  * [WidgetsApp], which inserts an [RouteOverlay] widget indirectly via its [Navigator].
///  * [MaterialApp], which inserts an [RouteOverlay] widget indirectly via its [Navigator].
///  * [CupertinoApp], which inserts an [RouteOverlay] widget indirectly via its [Navigator].
...
class RouteOverlay extends StatefulWidget {
  ...
  /// [RouteOverlayState] is initialized.
  ///
  ...
  /// Wrap the provided `child` in an [RouteOverlay] to allow other visual elements
  /// (packed in [OverlayEntry]s) to float on top of the child.
  ///
  /// This is a convenience method over the regular [RouteOverlay] constructor: It
  /// creates an [RouteOverlay] and puts the provided `child` in an [OverlayEntry]
  ...
  ///
  /// These entries are only used when the [RouteOverlayState] is initialized. If you
  /// are providing a new [RouteOverlay] description for an overlay that's already in
  /// the tree, then the new entries are ignored.
  ///
  /// To add entries to an [RouteOverlay] that is already in the tree, use
  /// [RouteOverlay.of] to obtain the [RouteOverlayState] (or assign a [GlobalKey] to the
  /// [RouteOverlay] widget and obtain the [RouteOverlayState] via
  /// [GlobalKey.currentState]), and then use [RouteOverlayState.insert] or
  /// [RouteOverlayState.insertAll].
  ///
  /// To remove an entry from an [RouteOverlay], use [OverlayEntry.remove].
  ...
  /// The [RouteOverlayState] from the closest instance of [RouteOverlay] that encloses
  ...
  /// `debugRequiredFor` argument) needs an [RouteOverlay] to be present to function.
  ...
  /// all subsequent instances of [RouteOverlay].
  ///
  /// This method can be expensive (it walks the element tree).
  ///
  /// See also:
  ///
  /// * [RouteOverlay.maybeOf] for a similar function that returns null if an
  ///   [RouteOverlay] is not found.
  static RouteOverlayState of(
    BuildContext context, {
    bool rootOverlay = false,
    Widget? debugRequiredFor,
  }) {
    final RouteOverlayState? result =
        maybeOf(context, rootOverlay: rootOverlay);
    assert(() {
      if (result == null) {
        final bool hiddenByBoundary =
            LookupBoundary.debugIsHidingAncestorStateOfType<RouteOverlayState>(
                context);
  ...
  /// See also:
  ///
  ///  * [RouteOverlay.of] for a similar function that returns a non-nullable result
  ///    and throws if an [RouteOverlay] is not found.

  static RouteOverlayState? maybeOf(
    BuildContext context, {
    bool rootOverlay = false,
  }) {
    return rootOverlay
        ? LookupBoundary.findRootAncestorStateOfType<RouteOverlayState>(context)
        : LookupBoundary.findAncestorStateOfType<RouteOverlayState>(context);
  }

  @override
  RouteOverlayState createState() => RouteOverlayState();
}

/// The current state of an [RouteOverlay].
///
/// Used to insert [OverlayEntry]s into the overlay using the [insert] and
/// [insertAll] functions.
class RouteOverlayState extends State<RouteOverlay>
  ...
  bool _debugCanInsertEntry(OverlayEntry entry) {
    final List<DiagnosticsNode> operandsInformation = <DiagnosticsNode>[
      DiagnosticsProperty<OverlayEntry>('The OverlayEntry was', entry,
          style: DiagnosticsTreeStyle.errorProperty),
      DiagnosticsProperty<RouteOverlayState>(
    ...
    final RouteOverlayState? currentOverlay = entry._overlay;
...
class _WrappingOverlay extends StatefulWidget {
  const _WrappingOverlay(
      {Key? key, this.clipBehavior = Clip.hardEdge, required this.child});
...
class _Theater extends MultiChildRenderObjectWidget {
  const _Theater({
    this.skipCount = 0,
    this.clipBehavior = Clip.hardEdge,
    required List<_OverlayEntryWidget> children,
  })  : assert(skipCount >= 0),
...
class _TheaterElement extends MultiChildRenderObjectElement {
  _TheaterElement(_Theater widget) : super(widget);
...
mixin _RenderTheaterMixin on RenderBox {
  ...
  static double? baselineForChild(
    ...
    final parentData = childParentData;
    double y;

    if (parentData.top != null) {
      y = parentData.top!;
    } else if (parentData.bottom != null) {
      y = theaterSize.height -
          parentData.bottom! -
...
class _TheaterParentData extends StackParentData {
  ...
  /// [RouteOverlay]. Currently, to make sure the overlay child is painted after its
  ...
  /// in the host [RouteOverlay], the paint order of each overlay child is managed by
...
class OverlayPortalController {
  ...
  /// to, at the top of the target [RouteOverlay].
  ///
  /// When there are more than one [OverlayPortal]s that target the same
  /// [RouteOverlay], the overlay child of the last [OverlayPortal] to have called
...
/// A widget that renders its overlay child on an [RouteOverlay].
///
...
/// specified [RouteOverlay] as if it was inserted using an [OverlayEntry], while it
/// can depend on the same set of [InheritedWidget]s (such as [Theme]) that this
/// widget can depend on.
///
/// This widget requires an [RouteOverlay] ancestor in the widget tree when its
/// overlay child is showing. The overlay child is rendered by the [RouteOverlay]
...
/// ### Paint Order
///
/// In an [RouteOverlay], an overlay child is painted after the [OverlayEntry]
...
/// to [OverlayPortal] instead of the target [RouteOverlay]. An [OverlayPortal]'s
...
/// [OverlayEntry] builds its widget subtree as a child of the target [RouteOverlay],
...
/// reparenting. If the content to be shown on the [RouteOverlay] doesn't benefit
...
///    relation to the target [RouteOverlay]'s boundaries.
...
class OverlayPortal extends StatefulWidget {
  ...
  /// builds on the closest [RouteOverlay] when [OverlayPortalController.show] is
  /// called.
  const OverlayPortal({
    Key? key,
  ...
  /// Creates an [OverlayPortal] that renders the widget [overlayChildBuilder]
  /// builds on the root [RouteOverlay] when [OverlayPortalController.show] is
  /// called.
  ...
  /// that renders on the closest [RouteOverlay].
  ///
  /// The said widget will only be built and shown in the closest [RouteOverlay] once
  ...
  /// the [RouteOverlay] on which it is rendered.
...
/// A location in an [RouteOverlay].
///
/// An [_OverlayEntryLocation] determines the [RouteOverlay] the associated
/// [OverlayPortal] should put its overlay child onto, as well as the overlay
/// child's paint order in relation to other contents painted on the [RouteOverlay].
//
...
class _RenderTheaterMarker extends InheritedWidget {
  ...
  static _RenderTheaterMarker of(BuildContext context,
    ...
    throw FlutterError.fromParts(<DiagnosticsNode>[
      ...
      ...context.describeMissingAncestor(expectedAncestorType: RouteOverlay),
    ]);
  }
...
class _OverlayPortal extends RenderObjectWidget {
  /// Creates a widget that renders the given [overlayChild] in the [RouteOverlay]
...
class _OverlayPortalElement extends RenderObjectElement {
  _OverlayPortalElement(RenderObjectWidget widget) : super(widget);
