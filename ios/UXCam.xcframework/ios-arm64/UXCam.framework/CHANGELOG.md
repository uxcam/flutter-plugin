...
Version   | Changes
---------- | ----------
3.8.2    | Fixed `pauseScreenRecording`, `occludeSensitiveScreen` misbehaviour during session start and orientation touchpoint.
           Updated upload flow for better visibility.
           Improvements in crash and intergration log.
3.8.1    | Optimized video recording flow 
3.8.0    | Schematic Recording information is shared in datafile. 
         | `optIntoSchematicRecordings`, `optOutOfSchematicRecordings` and `optInSchematicRecordingStatus` are deprecated with new APIs `optIntoVideoRecordings`, `optOutOfVideoRecordings` and `optInVideoRecordingStatus`. 
         | AppVersion and AppVersionBuildNumber are separated.
         | Improved uploading sessions logic to reduce missing sessions.
3.7.12   | Fixed crash related to NSFileHandleOperationException, NSInvalidArgumentException, internal logging, setupDisplaylink and status bar orientation. 
           Updated occlusion metadata.
3.7.11   | Optimized crash symbolication report to indicate crashed line.
3.7.10   | Reduce image capture frequency for video generation to improve performance. Fix crash issue due to upload network exception.
3.7.9   | Fixed crash on Webview Instrumentation and on frame consumer due to off queue mutation during active processing. Fixed sessions failing to upload due to file path change. Fixed occlusion skipping for first frame of web view.
3.7.8   | Fixed crash while occluding search field in SwiftUI
3.7.7   | Fixed crash due to allowShortBreak
3.7.6   | Fixed occlusion mismatch issue on webviews
