...
/**
 * @brief Enable Flutter-orchestrated, synchronized frame capture (iOS only).
 *
 * When YES (the default on iOS), Flutter captures occlusion rects on the
 * same frame the screenshot is taken, eliminating the time gap between
 * rect collection and the bitmap grab. Has no effect on UIKit-only apps
 * (where the Flutter bridge is never attached) and falls back to the
 * legacy pull-based capture path automatically if the Flutter plugin
 * predates this feature.
 *
 * Set to NO to force the legacy `pauseRendering` + `requestAllOcclusionRects`
 * capture flow on a build of the plugin that does support synchronization
 * — useful as an escape hatch if the synchronized path misbehaves.
 */
@property (nonatomic, assign) BOOL enableSynchronizedCapture;
